...
STABILITY="stable"
COMPOSER_INSTALL_VER=dev-master
PHP_TOO_HIGH_VER=7.4
PHP=$(which php)
...
if [[ $COMPOSER_INSTALL_VER == "" ]] ; then
    ...
    COMPOSER_INSTALL_SETUP="${MAJOR_MINOR_VER}.*"
    BOLT_INSTALL_SETUP="${MAJOR_MINOR_VER}.*"
elif [[ $STABILITY == "stable" ]] ; then
    COMPOSER_INSTALL_REQUIRE="^$MAJOR_MINOR_VER"
    BOLT_INSTALL_REQUIRE="^${BASE_BOLT_VER}"

    COMPOSER_INSTALL_SETUP="${MAJOR_MINOR_VER}.*"
    BOLT_INSTALL_SETUP="${BASE_BOLT_VER}.*"
elif [[ $STABILITY == "beta" ]] ; then
    COMPOSER_INSTALL_REQUIRE="^$MAJOR_MINOR_VER"
    BOLT_INSTALL_REQUIRE="^${BASE_BOLT_VER}@beta"

    COMPOSER_INSTALL_SETUP=$COMPOSER_INSTALL_REQUIRE
    BOLT_INSTALL_SETUP=$BOLT_INSTALL_REQUIRE
else
